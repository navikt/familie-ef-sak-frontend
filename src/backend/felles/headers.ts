...
const navReopsEventProxy = 'https://reops-event-proxy.nav.no';
const navReopsEventProxyDev = 'https://reops-event-proxy.ekstern.dev.nav.no';

const cspString = `default-src 'self' data: ${sentry} ${navTelemetry} ${navTelemetryDev} ${navCdn} ${navUmami} ${navReopsEventProxy} ${navReopsEventProxyDev}; style-src 'self' ${styleSource} data: 'unsafe-inline'; font-src 'self' ${fontSource} ${navFontSource} data:; frame-src 'self' blob:;`;
